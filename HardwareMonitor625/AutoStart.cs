...
namespace HardwareMonitor625
{
    class AutoStart
    {
        ...
        public void SetMeAutoStart(bool onOff)
        {
            if (onOff)//开机启动
            {
                //获取启动路径应用程序快捷方式的路径集合
        ...
        ///  获取是否自动开机
        /// </summary>
        public bool GetAutoSatrtMode()
        {
            List<string> shortcutPaths = GetQuickFromFolder(systemStartPath, appAllPath);
            for (int i = 0; i < shortcutPaths.Count; i++)
            {
                Console.WriteLine(shortcutPaths[i]);
                if (shortcutPaths[i].Substring(shortcutPaths[i].LastIndexOf("\\") + 1) == QuickName + ".lnk")
                {
                    return true;
                }
            }
